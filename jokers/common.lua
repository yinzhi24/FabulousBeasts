...
SMODS.Joker({
    ...
    loc_vars = function(self, info_queue, card)
        return {
            vars = {
                card.ability.extra.xmult or 1.1
            }
        }
    end,
    ...
    calculate = function(self, card, context)
        ...
        and context.other_card:get_id() == 14 then
            return {
                x_mult = card.ability.extra.xmult or 1.1,
...
SMODS.Joker({
    key = "casting_portrait",
    loc_txt = {
        name = "Casting Portrait",
        text = {
            "{C:mult}+#1#{} Mult for each",
            "{C:attention}Vanilla Joker{} you have",
            "{C:inactive}Currently {C:mult}+#2#{} Mult{}"
        }
    },
    atlas = "jokers",
    pos = { x = 0, y = 1 }, -- change sprite slot
    rarity = 1,
    cost = 5,
    discovered = true,
    unlocked = true,
    ...
    loc_vars = function(self, info_queue, card)
        ...
        for _, joker in ipairs(G.jokers and G.jokers.cards or {}) do
            ...
            if center and center.set == "Joker"
            and center.key
            and not center.key:find("^j_fb_") then
                n = n + 1
            end
        end
        ...
        return {
            vars = {
                card.ability.extra.mult_per_vanilla,
                n * card.ability.extra.mult_per_vanilla
            }
        }
    end,
    calculate = function(self, card, context)
        if FB.is_scoring_joker_main(context) then
            ...
            return {
                mult = n * card.ability.extra.mult_per_vanilla,
                card = card
            }
        end
    end
...
SMODS.Joker({
    ...
    loc_txt = {
        name = "Dog Food",
        text = {
            "First scored card adds",
            "{C:chips}+#1#X{} its current",
            "{C:chips}Chip{} value this hand"
        }
    },
    atlas = "jokers",
    pos = {
...
SMODS.Joker({
    ...
    calculate = function(self, card, context)
        ...
        if FB.is_scoring_joker_main(context) then
            ...
            if FB.is_final_hand() then
                return {
                    chips = card.ability.extra.stored * 2
                }
            end
        end
    end
...
SMODS.Joker({
    ...
    loc_txt = {
        ...
        text = {
            "At blind start, trigger {C:attention}Lunchbox Medkit{}",
            "once for each open Joker slot"
        }
    },
    atlas = "jokers",
    pos = {
...
SMODS.Joker({
    ...
    calculate = function(self, card, context)
        ...
        if context.after and not context.blueprint and(card.ability.extra.uses or 0) > 0 then
            ...
            return {
                message = tostring(card.ability.extra.uses).." left",
                colour = G.C.ATTENTION,
                card = card
            }
        end
    ...
    calculate = function(self, card, context)
        if context.setting_blind and not context.blueprint and not card.ability.fb_used then
            ...
            if target then
                card.ability.fb_used = true
                local was_eternal,
                destroyed = FB.queue_destroy_after_removing_eternal(target)
                FB.queue_self_destroy(card)
                FB.resolve_or_defer_queued_actions(context)
                return {
                    message = was_eternal and "Mortified!",
                    colour = was_eternal and G.C.ETERNAL,
                    card = card
                }
            end
            return {
...
SMODS.Joker({
    ...
    loc_txt = {
        ...
        text = {
            "If {C:chips}Chips{} or {C:mult}Mult{} is greater",
            "than the blind requirement, give",
            "{X:consumable,C:white}^#1#{} Mult"
        }
    },
    atlas = "jokers",
    pos = {
    ...
    calculate = function(self, card, context)
        if FB.is_scoring_joker_main(context) and G.GAME and G.GAME.blind and(FB.num(hand_chips, 0) > FB.num(G.GAME.blind.chips, 0) or FB.num(mult, 0) > FB.num(G.GAME.blind.chips, 0)) then
            return {
                e_mult = card.ability.extra.emult or 4,
                card = card
            }
        end
    end
...
SMODS.Joker({
    ...
    loc_vars = function(self, info_queue, card)
        return {
            vars = {
                card.ability.extra.required_cards or 1
            }
        }
    end,
    calculate = function(self, card, context)
...
SMODS.Joker({
    ...
    add_to_deck = function(self, card, from_debuff)
        ...
        if G.jokers and G.jokers.cards then
            for _,
            other_joker in ipairs(G.jokers.cards) do
...
SMODS.Joker({
    ...
    calculate = function(self, card, context)
        ...
        if FB.is_card_repetition(context) and context.cardarea == G.play and context.scoring_name == card.ability.extra.hand then
            return {
                repetitions = 1,
                card = card
            }
        ...
        if(context.after or context.pre_discard) and not context.blueprint then
            card.ability.extra.hand = FB.random_poker_hand('paystub')
        end
    end
})
    ...
    loc_vars = function(self, info_queue, card)
        return {
            vars = {
                ...
                card.ability.extra.destroy_cap
            }
        }
    end,
    calculate = function(self, card, context)
        if context.after and not context.blueprint and(card.ability.extra.uses or 0) > 0 and G.play and G.play.cards and #G.play.cards > 0 then
            ...
            destroy_count do
                ...
                local target,
                index = pseudorandom_element(pool, pseudoseed('rat_poison_'..tostring(i)))
                if not index then
                    for j,
                    c in ipairs(pool) do
                        if c == target then
...
SMODS.Joker({
    ...
    loc_txt = {
        ...
        text = {
            ...
            "{C:attention}Straight{} (not {C:attention}Straight Flush{})"
        }
    },
    atlas = "jokers",
    pos = {
...
SMODS.Joker({
    ...
    calculate = function(self, card, context)
        ...
        if FB.is_scoring_joker_main(context) then
            return {
                chips = card.ability.extra.chips or 233
            }
        end
...
SMODS.Joker({
    ...
    loc_txt = {
        name = "Tulou",
        text = {
            "{C:red}Destroys{} a random played card."
        }
    },
    atlas = "jokers",
    pos = {
...
SMODS.Joker({
    ...
    calculate = function(self, card, context)
        ...
        and context.other_card:get_id() == 2 then
            return {
                chips = card.ability.extra.mult or 12,
                card = context.other_card
            }
        end
    end
...
SMODS.Joker({
    ...
    loc_txt = {
        ...
        text = {
            "When sold, gain {C:money}$#1#{}-{C:money}$#2#{}"
        }
    },
    atlas = "jokers",
    pos = {
