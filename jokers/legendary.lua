...
SMODS.Joker({
    ...
    loc_txt = {
        name = "Bajin",
        text = {
            "{X:chips,C:white}X#2#{} Chips per {C:attention}Food{} Joker",
    ...
    loc_vars = function(self, info_queue, card)
        ...
        return {
            vars = {
                math.max(1, (card.ability.extra.xchips_per_food or 8) * n),
                card.ability.extra.xchips_per_food or 8
            }
        }
    end,
    calculate = function(self, card, context)
        if FB.is_scoring_joker_main(context) then
            return {
                x_chips = math.max(1, (card.ability.extra.xchips_per_food or 8) * FB.count_food_jokers_explicit()),
                card = card
            }
        end
    end
...
SMODS.Joker({
    ...
    loc_txt = {
        name = "Bilibili",
        text = {
            "{C:attention}Retrigger{} non-Legendary Jokers",
            "Does not target {C:attention}Exotic{} or {C:attention}Divine{} Jokers"
        }
    },
    atlas = "jokers",
    pos = {
...
SMODS.Joker({
    ...
    calculate = function(self, card, context)
        if FB.is_scoring_joker_main(context) then
            return {
                x_mult =(card.ability.extra.base or 1) + FB.count_deck_mods() *(card.ability.extra.gain or 1),
                card = card
            }
        end
    end
...
SMODS.Joker({
    ...
    loc_vars = function(self, info_queue, card)
        return {
            vars = {
                card.ability.extra.hand_size_mult or 2
            }
        }
    end,
    add_to_deck = function(self, card, from_debuff)
...
SMODS.Joker({
    ...
    add_to_deck = function(self, card, from_debuff)
        FB.safe_change_joker_slots(card.ability.extra.joker_slots or 2);
        if G.consumeables and G.consumeables.config then
            G.consumeables.config.card_limit = G.consumeables.config.card_limit +(card.ability.extra.consumable_slots or 1)
        end
    end,
    remove_from_deck = function(self, card, from_debuff)
        FB.safe_change_joker_slots(-(card.ability.extra.joker_slots or 2));
...
SMODS.Joker({
    ...
    calculate = function(self, card, context)
        if context and context.retrigger_joker_check and context.other_card and not context.end_of_round and not context.setting_blind and not context.before and not context.after and not context.selling_card and not context.selling_self and not context.destroy_card and not context.remove_playing_cards and context.other_card ~= card and FB.is_food_joker(context.other_card) and FB.once_joker_retrigger(card, context, 'fuzai') then
            return {
                repetitions = card.ability.extra.repetitions or 3,
                card = card
            }
        end
    end
...
SMODS.Joker({
    ...
    loc_vars = function(self, info_queue, card)
        return {
            vars = {
                card.ability.extra.album_xchips or 631,
                card.ability.extra.alt_bonus or 6100
            }
        }
    end,
    calculate = function(self, card, context)
        if FB.is_scoring_joker_main(context) then
            ...
            if albums > 0 then
                ret.x_chips =(card.ability.extra.album_xchips or 631) ^ albums
            end
            if alts > 0 then
                ret.chips =(card.ability.extra.alt_bonus or 6100) * alts
                ret.mult =(card.ability.extra.alt_bonus or 6100) * alts
            end
            if ret.x_chips or ret.chips or ret.mult then
                return ret
            end
        end
    end
...
SMODS.Joker({
    ...
    loc_vars = function(self, info_queue, card)
        return {
            vars = {
                card.ability.extra.repetitions or 3
            }
        }
    end,
    calculate = function(self, card, context)
...
SMODS.Joker({
    ...
    calculate = function(self, card, context)
        ...
        if FB.is_scoring_joker_main(context) then
            return {
                e_mult = card.ability.extra.emult or 1,
                card = card
            }
        end
    end
...
SMODS.Joker({
    ...
    loc_txt = {
        name = "Shanque",
        text = {
            "{C:attention}Retrigger{} Chugou",
            "Played cards gain a random",
            "{C:attention}Seal{}, {C:attention}Enhancement{},",
            "or {C:attention}Edition{} they don't already have"
        }
    },
        ...
        if context.before
            ...
            for _, played_card in ipairs(G.play.cards) do
                ...
                if not played_card.seal then
                    modifier_types[#modifier_types + 1] = "seal"
                end

                if played_card.config
                    and played_card.config.center == G.P_CENTERS.c_base then
                    modifier_types[#modifier_types + 1] = "enhancement"
                end

                if not played_card.edition then
                    modifier_types[#modifier_types + 1] = "edition"
                end

                if #modifier_types > 0 then
                    local modifier_type = pseudorandom_element(
                        modifier_types,
                        pseudoseed("fb_shanque_modifier_type")
                    )

                    if modifier_type == "seal" then
                        local seals = {
                            "Red",
                            "Blue",
                            "Gold",
                        ...
                        played_card:set_seal(
                            pseudorandom_element(seals, pseudoseed("fb_shanque_seal")),
                            true
                        )
                    end
                    ...
                    if modifier_type == "enhancement" then
                        ...
                        local enhancement = pseudorandom_element(
                            enhancements,
                            pseudoseed("fb_shanque_enhancement")
                        )

                        if G.P_CENTERS[enhancement] then
                            played_card:set_ability(
                    ...
                    if modifier_type == "edition" then
                        ...
                        played_card:set_edition(
                            pseudorandom_element(editions, pseudoseed("fb_shanque_edition")),
                            true
                        )
                    end
                end
...
SMODS.Joker({
    ...
    calculate = function(self, card, context)
        if context.after and not context.blueprint and G.play and G.play.cards then
            local destroyed = 0
            for _,
            played_card in ipairs(G.play.cards) do
                if played_card then
...
SMODS.Joker({
    ...
    loc_txt = {
        ...
        text = {
            "Gain {X:mult,C:white}X#2#{} Mult for each",
            "{C:attention}Laurel Branch{} and {C:attention}Mooncake{}",
            "you have",
            "{C:inactive}Currently {X:mult,C:white}X#1#{} Mult{}"
        }
    },
    ...
    loc_vars = function(self, info_queue, card)
        ...
        return {
            vars = {
                (card.ability.extra.base or 1) + n *(card.ability.extra.gain or 1),
                card.ability.extra.gain
            }
        }
    end,
    calculate = function(self, card, context)
        ...
        if FB.is_scoring_joker_main(context) then
            local n = count_support_cards()
            return {
                x_mult =(card.ability.extra.base or 1) + n *(card.ability.extra.gain or 1),
                card = card
            }
        end
    end
...
SMODS.Joker({
    ...
    loc_txt = {
        ...
        text = {
            ...
            "one scoring card, give {X:mult,C:white}X#1#{} Mult",
            "and {X:chips,C:white}X#2#{} Chips"
        }
    },
    atlas = "jokers",
    pos = {
    ...
    calculate = function(self, card, context)
        if FB.is_scoring_joker_main(context) and context.scoring_hand and #context.scoring_hand == 1 then
            return {
                x_mult = card.ability.extra.xmult or 4,
                x_chips = card.ability.extra.xchips or 4,
                card = card
            }
        end
    end
