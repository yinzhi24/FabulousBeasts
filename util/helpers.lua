...
function FB.create_joker(base_key, edition, negative)
    ...
    if created and(edition or negative) then
        created: set_edition(negative and {
            negative = true
        }
        or edition, true)
    end
...
function FB.random_registered_key(seed)
    if not FB.joker_keys or #FB.joker_keys == 0 then
        return nil
    end
...
FB.is_food_joker = FB.is_food_joker or function(card)
    ...
    key = tostring(key)
    -- Vanilla food / food-adjacent Jokers keep their normal Balatro center keys.
    if FB.vanilla_food_joker_keys [key] == true then
        return true
    end
...
SMODS.current_mod and not FB.fb_optional_features_wrapped then
    FB.fb_optional_features_wrapped = true
    local fb_old_optional_features =
    ...
    SMODS.current_mod.optional_features = function()
        ...
        if type(fb_old_optional_features) == 'function' then
            ...
            if ok and type(old_features) == 'table' then
                for k, v in pairs(old_features) do
                    features [k] = v
                end
            end
        elseif type(fb_old_optional_features) == 'table' then
            for k, v in pairs(fb_old_optional_features) do
                features [k] = v
            end
        end
...
FB.round_condition_value = FB.round_condition_value or function(card, normal_key, boss_key, final_key)
    ...
    if FB.is_final_boss and FB.is_final_boss() then
        return final
    end
...
FB.clear_round_once = FB.clear_round_once or function(card, flag)
    if card and card.ability then
        card.ability [flag or 'fb_round_once'] = nil
    end
end
...
FB.queue_destroy = FB.queue_destroy or function(target_card)
    if target_card then
        FB.action_queue.destroy [#FB.action_queue.destroy + 1] = {
            card = target_card
        }
    end
end
FB.queue_self_destroy = FB.queue_self_destroy or function(source_card)
    if source_card then
        FB.action_queue.self_destroy [#FB.action_queue.self_destroy + 1] = {
            card = source_card
        }
    end
end
...
FB.resolve_queued_actions = FB.resolve_queued_actions or function()
    if not FB.queue_has_actions() then
        return
    end
...
FB.balance_score_return = FB.balance_score_return or function(card, args)
    ...
    return {
        ...
        mult = balanced - original_mult,
        colour = args.colour or G.C.PURPLE,
        card = card
    }
end
...
FB.is_copyable_joker = FB.is_copyable_joker or function(other_card)
    if not(other_card and other_card.config and other_card.config.center) then
        return false
    end
    if other_card.debuff then
        return false
    end
    if other_card.config.center.blueprint_compat ~= true then
        return false
    end
    if FB.is_beast_joker and FB.is_beast_joker(other_card) then
        return false
    end
...
FB.copy_single_joker_effect = FB.copy_single_joker_effect or function(card, other_card, context)
    ...
    SMODS.blueprint_effect then
        return

        SMODS.blueprint_effect(card, other_card, context)
    end
    return nil
end
...
FB.get_joker_to_right = FB.get_joker_to_right or function(card)
    ...
    if not index or not(G.jokers and G.jokers.cards) then
        return nil
    end
...
FB.main_end_of_round_once = FB.main_end_of_round_once or function(card, context, flag)
    if not(card and context and context.end_of_round) then
        return false
    end
    if context.blueprint or context.individual or context.repetition or context.before or context.after then
        return false
    end
    if context.cardarea and G and G.jokers and context.cardarea ~= G.jokers then
        return false
    end
    card.ability = card.ability or {}
    flag = flag or 'fb_end_of_round_once'
    local id = FB.current_round_id()
    if card.ability [flag] == id then
        return false
    end
...
FB.has_retrigger_effect = function(card)
    ...
    if center.retrigger or center.retriggered or center.retrigger_count then
        return true
    end
    if ability.retrigger or ability.retriggered or ability.retrigger_count then
        return true
    end
    if extra.retrigger or extra.retriggered or extra.retrigger_count then
        return true
    end
...
FB.once_joker_retrigger = function(source, context, tag)
    if not context or not context.retrigger_joker_check then
        return false
    end
    if not source or not context.other_card or context.other_card == source then
        return false
    end
    if context.end_of_round or context.setting_blind or context.before or context.after then
        return false
    end
    if context.selling_card or context.selling_self or context.destroy_card or context.remove_playing_cards then
        return false
    end
    ...
    ":"..tostring(source_id)..
    ">"..tostring(target_id)
    if FB.fb_rt_seen_temp [token] then
        return false
    end
...
FB.create_joker_any_key = function(key)
    if not key or FB.available_joker_slots() <= 0 then
        return nil
    end
    local actual = tostring(key)
    if not actual: match('^j_') then
        actual = FB.key(actual)
    end
    -- If the center does not exist, fail safely instead of crashing.
    if not(G and G.P_CENTERS and G.P_CENTERS [actual]) then
        print("[Fabulous Beasts] Missing Joker center:", actual)
        return nil
    end
    return
...
FB.create_lunchbox_food = function(seed)
    if FB.available_joker_slots() <= 0 then
        return nil
    end
    ...
    local pool = FB.weighted_choice(seed or 'lunchbox_rarity', {
        {
            weight = cw,
            value = FB.food_common_pool
        },
        {
            weight = uw,
            value = FB.food_uncommon_pool
        },
        {
            weight = rw,
            value = FB.food_rare_pool
        }
...
FB.has_life_saving_joker = function(except)
    for _, j in ipairs(FB.joker_cards()) do
        if j ~= except then
            ...
            if k == 'emergency_rations' or k == 'demolition_notice' or k == 'mr_bones' or k == 'j_mr_bones' then
                return true
            end
        end
    end
...
FB.set_better_edition = function(card, edition)
    ...
    if edition and edition.polychrome then
        r = 3
    elseif edition and edition.holo then
        r = 2
    elseif edition and edition.foil then
        r = 1
    end
    if r > FB.edition_rank(card) then
        card: set_edition(edition, true);
        return true
    end
    return false
...
FB.recharge_joker = FB.recharge_joker or function(card)
    if not card or not card.ability then
        return false
    end
    local key = FB.raw_key and FB.raw_key(card)
    or(FB.get_center_key and FB.get_center_key(card))
    if not key then
        return false
    end
    key = tostring(key): gsub("^j_fb_", "")
    ...
    if key == "chicken_mushroom_stew" then
        extra.servings = 15
        return true
    end
    -- Qilin Egg
    if key == "qilin_egg" then
        extra.rounds = 0
        return true
    end
    -- 9th Heaven
    if key == "9th_heaven" then
        extra.rounds = extra.needed or 9
        return true
    end
    -- Baby Tianlu
    if key == "baby_tianlu" then
        extra.chips = extra.evolve_chips or 1000
        return true
    end
    -- Baby Bixie
    if key == "baby_bixie" then
        extra.mult = extra.evolve_mult or 300
        return true
    end
    -- Qishiqi, if you keep it counter-based
    if key == "qishiqi" then
        extra.triggers = extra.needed or 77
        return true
    end
    return false
end
